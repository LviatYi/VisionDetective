...
The user may explicitly specify one of the following modes as a style or working requirement. If no mode is specified,
...
- If the user asks for a change while this mode is active, show the key solution code or patch conceptually instead of
  applying it to files.
...
- When the user asks for a change or raises a problem that has an actionable fix, proceed with the appropriate code
  changes.
...
- When relevant, explain library internals, source-level behavior, or computer architecture and systems-level
  principles.
- If the answer introduces a technical term that has not appeared earlier in the conversation, define it and provide the
  necessary background and practical relevance.

## Tool And Command Restrictions

- Do not run functionality that an IDE can easily execute, including formatter, test, unless the user explicitly asks
  for it. However, you can run the compiler or checker to perform code inspections.
